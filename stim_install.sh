...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='stim.conf'
CONFIGFOLDER='/root/.stimcore'
COIN_DAEMON='stimd'
COIN_CLI='stim-cli'
COIN_PATH='/usr/local/bin/'
COIN_REPO='https://github.com/STM0x0/stim.git'
COIN_TGZ='https://github.com/Realbityoda/Stim/releases/download/v1.0.0.0/stim.zip'
...
purgeOldInstallation() {
    ...
    #kill wallet daemon
    sudo killall stimd > /dev/null 2>&1
    #remove old ufw port allow
    sudo ufw delete allow 8093/tcp > /dev/null 2>&1
    #remove old files
    if [ -d "~/.stimcore" ]; then
        sudo rm -rf ~/.stimcore > /dev/null 2>&1
    fi
    #remove binaries and Stim utilities
    cd /usr/local/bin && sudo rm stim-cli stim-tx stimd > /dev/null 2>&1 && cd
...
function important_information() {
 echo
 echo -e "${BLUE}================================================================================================================================${NC}"
 echo -e "${PURPLE}Windows Wallet Guide. https://github.com/Realbityoda/Stim/blob/master/README.md${NC}"
 echo -e "${BLUE}================================================================================================================================${NC}"
